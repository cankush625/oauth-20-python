...
class Google:
    ...
    def sign_in(redirect_uri: str, private_key: str):
        ...
        return google_oauth.sign_in(redirect_uri, state_token)

    @staticmethod
    def oauth_callback(request: urllib.request.Request, state: str, redirect_uri: str):
        """
        ...
        if request.headers.get("state") != state:
            raise StateTokenDoesNotMatchError(
                {"StateToken": STATE_TOKEN_DOES_NOT_MATCH}
            )
        return google_oauth.get_access_token(redirect_uri, request.headers.get("code"))
